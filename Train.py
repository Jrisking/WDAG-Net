    ...
    try:
        for i, (images, gts, edges) in enumerate(train_loader, start=1):
            ...
            loss_init = structure_loss(preds[0], gts) * 0.0625 + structure_loss(preds[1], gts) * 0.125 + structure_loss(
                preds[2], gts) * 0.25 + structure_loss(preds[3], gts) * 0.5
            ...
            if i % 20 == 0 or i == total_step or i == 1:
                ...
                writer.add_scalars('Loss_Statistics',
                                   {'Loss_init': loss_init.data, 'Loss_final': loss_final.data,
                                    'Loss_edge': loss_edge.data, 'Loss_total': loss.data},
                                   global_step=step)
...
def val(test_loader, model, epoch, save_path, writer):
    ...
    with torch.no_grad():
        ...
        print('Epoch: {}, MAE: {}, Sm：{}, adpE: {}, wF: {}, bestMAE: {}, bestEpoch: {}.'.format(epoch, mae.round(4), sm, adpem, wfm, best_mae, best_epoch))
        if epoch == 1:
            best_mae = mae
            best_epoch = 1
        else:
            # if mae < best_mae and sm > 0.8 and adpem > 0.8 and wfm > 0.8:
            #     best_mae = mae
            #     best_epoch = epoch
            ...
            if mae < best_mae:
                ...
                print('Save state_dict successfully! Best epoch:{}.'.format(epoch))
        logging.info(
            '[Val Info]:Epoch: {}, MAE: {}, Sm：{}, adpE: {}, wF: {}, bestMAE: {}, bestEpoch: {}.'.format(epoch, mae.round(4), sm, adpem, wfm, best_mae, best_epoch))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_root', type=str, default='/home/ljh/ljh_COD/DRNet/Train/',
                        help='the training rgb images root')
    parser.add_argument('--val_root', type=str, default='/home/ljh/ljh_COD/DRNet/Test/',
                        help='the test rgb images root')
    parser.add_argument('--save_path', type=str, default='/home/ljh/ljh/PythonProject/net/snapshot/Base_FF/', help='the path to save model and log')
    ...
    train_loader = get_loader(image_root=opt.train_root + 'Imgs/',
                              ...
                              num_workers=8)
    # CAMO,  COD10K
    val_loader = test_dataset(image_root=opt.val_root + 'CAMO/' + 'Imgs/',
    ...
    cosine_schedule = optim.lr_scheduler.CosineAnnealingLR(optimizer=optimizer, T_max=20, eta_min=1e-5)  # 20
    print("Start train...")
